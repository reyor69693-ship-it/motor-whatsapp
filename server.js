...
client.on('qr', qr => { console.log("¡ESCANEA ESTE CÓDIGO QR CON TU WHATSAPP!"); qrcode.generate(qr, { small: true }); });
client.on('ready', () => { console.log('¡Cliente de WhatsApp está listo!'); });
client.initialize();
app.get('/', (req, res) => { res.send('El bot de WhatsApp está funcionando :)'); });
...
const listener = app.listen(process.env.PORT || 3000, () => { console.log('Tu app está escuchando en el puerto ' + listener.address().port); });
